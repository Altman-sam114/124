# WWIIHexV0 核心流程文档（v3.7-preflight.117 隋唐迁移）
...
### 0.1 v3.0-v3.7-preflight.117 隋唐迁移状态

当前已存在隋末唐初迁移总提示词、v3.0 审计合同、v3.1 最小兼容迁移记录、v3.2 默认数据迁移记录、v3.3 战争规则迁移记录、v3.4 朝堂 AI 分层记录、v3.5 玩家体验记录、v3.6 UI 收口记录，以及 v3.7-preflight 至 v3.7-preflight.117 的发布候选前置记录。最近阶段已完成 RegionVictoryRules 隋唐胜负摘要对齐、共享隋唐胜负 evaluator 收口、指令结果语义化固守判定收口、阶段与旧总管展示口径收口、自动方面总管默认指挥风格收口、默认指挥风格共享 helper 收口、DataLoader 场景阶段兜底收口、legacy phase 存档规范化收口、动态方面推进势力兜底收口、RegionDataSet owner/controller 兜底收口、ScenarioSemantics 场景语义和胜负 fallback 门禁收口、MapEditor 非法 unit faction 导入诊断收口、归附善后治安压力落地、归附善后贡赋效率落地、归附善后忠诚/叛乱/俘虏/安置只读审计、渡口港口粮道补给减免落地、MapEditor 河边数据往返保真、MapEditor 河边绘制/擦除入口、受控渡口港口补给投送点、默认水路地点河边资产补录、己控渡口港口移动渡河减免、隔河近战水路控制校验、己控港口海港生产部署点、己控港口海港战略补给源、玩家可见接触态势术语古代化、骑兵基础机动/冲击强化、水路补给 anchor 粮道显示对齐、名将军令署名显示，以及 AI 粮草目标权重：
...
v3.7-preflight.106 已把己控渡口/港口地点接入战术补给投送：`SupplyRules` 内部用 `effectiveSupplyAnchors(for:in:)` 合并既有 `supplySources` 和己方控制的 `MapFeatureKind.ferry` / `port` / `harbor` 坐标；这些 anchor 只参与单位补给线、安全撤退和撤退排序。该层不写入 `MapState.supplySources`，不改省级 `RegionSupplyRules`、经济部署点、JSON schema、移动、战斗或水战规则。

v3.7-preflight.107 已补录默认水路地点河边资产：`wude_618_scenario.json` 为蒲津渡 `(4,3)` 写入 `northEast`，为孟津渡 `(6,4)` 在既有 `east` 外补入 `northEast`，为洛口津 `(6,6)` 写入 `west`，黎阳津 `(7,4)` 保留既有 `east`。四个默认 ferry / port 地点自身 hex 均有非空 `riverEdges`，供既有跨河补给、补给 anchor 和移动渡河规则消费。该层不改 JSON schema、地图尺寸、地点、单位、运行时战斗/补给算法，也不自动双写邻接 hex 反向河边。

v3.7-preflight.108 已把己控水路通行点接入战术移动渡河成本：`MovementRules.shortestPaths` 内部使用带 `GameState` 和移动方势力的成本函数。非道路跨 `riverEdges` 行军时，若跨河两端任一 hex 有 `MapFeatureKind.ferry` / `port` / `harbor` marker 且该 hex `controller == movingFaction`，该段渡河额外移动成本降为 0；敌控或未控水路点不提供减免。该层保留公开 `movementCost(from:to:direction:)` 的纯地形语义，不改命令、JSON schema、MapEditor、补给、战斗、水战或部署点。

v3.7-preflight.109 已把己控港口/海港接入生产部署 fallback：`EconomyRules.deploymentHex(for:preferredRegionId:in:)` 仍优先选择合格己控州郡和永久 `SupplySource`，若都不可用，再选择己控、可通行、无驻军、非敌邻接的 `MapFeatureKind.port` / `harbor` marker 所在 hex 部署新征发部队。该层不让 `ferry` 成为募兵部署点，不写入 `MapState.supplySources`，不改补给线、收入、移动、战斗、水战、命令或 JSON schema。
...
v3.7-preflight.116 已把名将存在感接入军令面板和提交反馈：`GeneralCommandPanelView` 标题改为“名将军令”，预备军令使用 `PlayerPlannedOperation.createdByGeneralId` 经 `GeneralRegistry` 解析署名；可解析时显示“某将军令：进军/固守：目标”，旧记录或缺失将领时保守回落“总管军令”。`RootGameView` 把 `AppContainer.generalRegistry` 传入面板，`AppContainer` 的玩家军令提交反馈也会显示“某将军令已提交”。该层只改 UI 展示和调用传参，不改变命令管线、存档 schema、AI 决策、规则执行、JSON 数据或将领技能消费。

v3.7-preflight.117 已把粮草权重接入 AI 目标选择和说明：`ZoneCommanderAgent` 的进攻候选和突破 focus 都把 `supplyValue` 按三倍计入价值；`MarshalBattlefieldSummary` schemaVersion 6 为每个方面摘要高粮草 / 粮仓目标，`SimulatedMarshalLLMClient` 会优先把这些目标放入 `weightedRegions` / `focusRegionId`，并在 rationale / strategicIntent 中说明粮仓粮道理由；`AgentPromptBuilder` 让 legacy LLM prompt 显示州郡粮草值和“粮仓要地”标记；`MockAIClient` 的攻击、部署攻击和行军目的地排序也纳入粮草值，并生成夺粮草、压迫补给或沿粮道逼近粮仓的中文 reason。该层只改变 AI 决策权重和自由文本，不改变命令 schema、补给规则、移动、战斗、水战、部署、JSON 数据或真实 LLM 接入。

古代作战口径仍按长期原则推进：玩家可见和 AI rationale 不应把隋唐战场做成现代连续战线，应突出天下局势、边境接触、行军道、关隘、粮仓、渡口、骑军冲击、粮道压力和英雄人物；可参考《三国志14》等古代战略游戏的局势、粮道、人物和据点体验，但只能作为设计启发，不照搬资产、文本、数值或受版权保护内容。

仍未完成的迁移边界：README / AGENTS 项目身份仍按真实工程历史保留，完整天命/民心、完整水战/河面控制/运输、siege progress、真实多模型协作、归附交接后的实际叛乱触发/俘虏处置/安置命令、正式地图资产替换决策、完整 UI 文案穷尽审计、骑兵技能/名将统率消费、真实截粮 / 粮道封锁 / 逐格粮道目标和完整发布候选运行时重测流程尚未迁移。
...
- v3.7-preflight.106 起，己控渡口、港口、海港可作为战术补给投送点，但不等同于永久 `SupplySource` 或部署点。
- v3.7-preflight.107 起，默认 `wude_618` 四个水路地点自身 hex 均有非空 `riverEdges`；这是地图资产输入。
