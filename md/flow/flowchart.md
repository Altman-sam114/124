...
## 0.1 v3.0-v3.7-preflight.105 隋唐迁移入口

这张图说明当前迁移状态已推进到 v3.7-preflight.105；最近几轮收口动态方面推进势力兜底、RegionDataSet owner/controller 兜底、场景语义 helper、胜负 fallback 门禁、MapEditor unit faction 导入诊断、归附善后治安压力、贡赋效率、渡口港口粮道补给、MapEditor 河边数据保真和河边绘制入口，让异常缺 zone 路径不再把推进方静默落到旧东路势力，非 legacy region 数据缺 owner 时不再静默落到旧西路势力，未知自定义场景不再静默套用旧胜负规则，MapEditor 坏 unit faction 也不再静默变成旧西路势力，归附交接后的善后风险也会写入受影响州郡的治安/顺从状态并影响后续府库收入，渡口/港口地点也会影响跨河补给成本，MapEditor 默认资源读写和编辑入口也不再丢失 tile 河边。
