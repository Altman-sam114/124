...
当前已新增 v3.0-v3.7-preflight.92 迁移记录：
...
v3.7-preflight.44 已收口 legacy MockAI intent/reason、本地模拟总管摘要、元帅 directive 解码错误和战报诊断净化中的可见英文工程口径。v3.7-preflight.45 已收口旧 fallback 场景名、dataNotes、城邑/补给点名和 legacy 单位模板 displayName 中的可见英文数据口径。v3.7-preflight.46 已收口 legacy 将领军衔、履历、总管展示名和技能显示中的可见英文/未知兜底。v3.7-preflight.47 已收口 Agent 角色显示、面板诊断净化、legacy 映射失败和数据加载校验失败中的可见英文/异常兜底。v3.7-preflight.48 已收口自动回合、元帅配置、legacy 映射失败和元帅解析/编译失败中的可见英文/异常兜底。v3.7-preflight.49 已收口数据加载初始战报、MapEditor 州郡数据标题和 scenario dataNotes 中的可见英文/工程词。v3.7-preflight.50 已收口战局复核、MapEditor、战报、朝堂和外交面板中的发布/工程口径与历史摘要直出风险。v3.7-preflight.51 已收口 MapEditor 和主游戏详情/记录中的 raw 坐标、文件名、目录名、raw objective id、方面/防区 raw name 和通用 raw id 展示风险。v3.7-preflight.52 已收口命令展示名、州郡命令展示名、RuleEngine 成功消息、命令意图适配错误、元帅军令解码错误、legacy 映射错误、行军/撤退/推进/拒绝/控制权变化战报、总管防区展示和诊断 raw agent id 净化风险。v3.7-preflight.53 已收口自动总管展示名和将领档案所属防区中的 raw 防区 id/name 展示风险。v3.7-preflight.54 已收口 legacy fallback 场景、地点、州郡、城邑和将领履历中的草稿名、坐标式名称和旧剧本口径展示风险。v3.7-preflight.55 已收口 legacy fallback 初始单位名、阿登战局名、dataNotes 和防区名中的泛化、预检与州郡口径展示风险。v3.7-preflight.56 已收口静态阿登 fallback 地点、目标、单位、初始化战报和胜负原因展示风险，并让 legacy 目标查找以 objective id 优先。v3.7-preflight.57 已收口旧 LocalLLM prompt、JSON 示例值和古德里安 fallback 配置中的英文输出诱导风险。v3.7-preflight.58 已收口外交面板势力/盟从名称直出英文风险，并补齐外交记录摘要中的内部词和常见审计 id 净化。v3.7-preflight.59 已收口战报面板 v3.x/复合 intent 直出、半清洗审计 id 和中文战报分类降级风险。v3.7-preflight.60 已收口 MapEditor 普通导出 metadata fallback，使未知自定义文档默认使用隋唐草稿口径，只有明确 legacy / Ardennes / WWII / 阿登 / 旧战局时才使用阿登元数据。v3.7-preflight.61 已收口 AI 源头诊断与面板展示净化口径，使 `model`、`legacy pipeline`、`RuleEngine` 和常见审计 id 统一转为玩家语义。v3.7-preflight.62 已收口 legacy `general_agents.json` 中 `guderian` 展示字段和 `GameAgent(definition:)` 旧数据兜底，避免英文总管名或英文作战偏好继续进入旧本地军议上下文。v3.7-preflight.63 已收口旧 `AgentPromptBuilder` 的 prompt 结构，让中文战场摘要、内部编号和 JSON 解析合同分层展示，降低 legacy LocalLLM 把 raw id 或英文 type 回写到中文摘要字段的风险。v3.7-preflight.64 已收口旧 `AgentPromptBuilder` 中近期战报和玩家意图的直通文本，进入 legacy prompt 前先做局部 raw id / 工程词净化。v3.7-preflight.65 已收口旧 `AgentPromptBuilder.systemPrompt` 中决策者和性格自由文本直通风险，同时让 sample agent 默认 personality 中文化。v3.7-preflight.66 已收口 legacy `MockAIClient` 生成的英文 stance 自由文本，避免旧 MockAI 回归路径继续写入英文姿态。v3.7-preflight.67 已收口旧 `AgentPromptBuilder` 面向 LocalLLM 的 prompt 工程说明词，把 `hex`、`schema`、`Markdown` 和 `JSON schema` 说明转为六角格、结构化输出和排版标记口径，同时保留 JSON 字段与 parser / mapper 合同。v3.7-preflight.68 已收口 `SimulatedMarshalLLMClient` 的模拟元帅输出包装，改为直接返回纯 JSON，decoder 仍兼容 fenced JSON 和纯 JSON。v3.7-preflight.69 已收口 AI 面板、战报面板和外交面板的记录净化 helper 顺序与词表，使三处均先清 raw id，再替换工程词、模型词和记录词。v3.7-preflight.70 已收口 legacy 将领 prompt、技能展示、朝堂 rationale 和结构化军令标题中的旧题材 / 工程词。v3.7-preflight.71 已收口 CommandPanel 命令消息展示净化，使 `lastCommandMessage` 进入命令面板前先清 raw id，再替换工程词、模型词和 hex 词。v3.7-preflight.72 已收口单位详情与提示中的 legacy 单位名展示，使单位详情、tooltip 可见标题和 VoiceOver label 同步使用展示净化名。v3.7-preflight.73 已收口州郡详情中的 legacy 地名、目标名和驻军名展示，使 `RegionInspectorView` 的州郡、城邑、方面、防区、要地和军队列表先走展示净化。v3.7-preflight.74 已收口将领与总管面板中的 legacy 文案展示，使 `GeneralProfileView` 和 `GeneralCommandPanelView` 的将领、军衔、履历、防区、军队和目标州郡先走展示净化。v3.7-preflight.75 已收口 `MapDisplayAdapter` / SpriteKit 地图展示入口，使地图标签和详情 objective 先走 adapter 层净化，详情面板既有 id / faction 展示则在 UI 层兜底净化，且不扩展 inspector state 合同。v3.7-preflight.76 已收口 MapEditor 选择器与状态消息展示，使州郡 / 方面选择器和地点状态消息先走 MapEditor 局部展示净化，且不改变文档数据、导出 JSON 或 id 合同。v3.7-preflight.77 已收口 AppContainer 交互日志与存档反馈展示，使存档、选择、军队、州郡、防区、外交 / 归附命令标题和 fallback 总管名称先走局部展示净化，且不改变存档 schema、`Command.displayName` 合同、数据层名称、AI 决策或规则。v3.7-preflight.78 已收口 GameLogEntry 源头战报展示，使 Rules / Commands / Data / Core 的 appendEvent 和直接 GameLogEntry 构造路径在保存 message 前净化 legacy 文案 / raw id，且不改变 eventLog schema、relatedRecordId、规则数值、命令执行或数据层名称。v3.7-preflight.79 已收口 legacy LocalLLM prompt 临时编号别名，使旧 prompt 中的势力、目标、军队、州郡和编号旁说明名先净化，并用临时编号替代真实 id，解析后再回填真实 id，且不改变 `AgentDecision` schema、JSON 字段名、type rawValue、parser / mapper、默认 AI 主链路或规则。v3.7-preflight.80 已收口 legacy fallback 行军总管配置，使旧 `guderian` fallback、legacy marshal config、模拟军议 id 前缀和可见诊断不再直出旧二战人物、旧势力词或 `marshal_*` 口径，且不改变 marshal / directive schema、Faction enum、JSON 数据、命令管线、规则或存档。v3.7-preflight.81 已收口朝堂/外交实际记录 id 展示净化，使 `TurnManager`、可见 UI 面板、源头战报和旧 LocalLLM prompt 均覆盖真实 `ruler_*_turn_*`、`court_*_turn_*`、`court_<turn>_*`、`diplomacy_<turn>_*` 记录 id，并把外交摘要修正为敌对国家计数，且不改变记录 id、Codable schema、`relatedRecordId`、外交判定、命令管线、规则或存档。v3.7-preflight.82 已收口行军总管可见称谓净化，使 Field Marshal / Guderian / MarshalDirective 在 UI 展示和角色展示中统一为行军总管、历史总管或军议，并补齐 directive/order/agent id 展示净化，且不改变 rawValue、provider suffix、record id、schema、命令管线、规则或存档。v3.7-preflight.83 已收口源头 legacy 中文势力/国家/地名展示净化，使 `TurnManager`、`DiplomacyState` 和 `GameLogEntry` 的源头展示文本补齐旧中文势力、旧国家、旧地名、旧单位词和工程词净化，且不改变数据、rawValue、record id、schema、命令管线、规则或存档。v3.7-preflight.84 已收口将领档案/总管军令称谓净化对齐，使 `GeneralProfileView` 和 `GeneralCommandPanelView` 的 `Field Marshal`、`Guderian / 古德里安` 与空单位名 fallback 展示口径对齐，且不改变 `GeneralData`、`Division.name`、`AgentRole` rawValue、JSON、命令管线、规则或存档。v3.7-preflight.85 已收口 fallback JSON 可见数据文本，使旧 fallback 场景、单位、地点、粮站、region、单位模板和将领可见字段改为迁移期中性口径，且不改变 JSON key、id、rawValue、schema、坐标、faction、templateId、加载顺序、命令管线、规则或存档。v3.7-preflight.86 已收口源码层 legacy 可见兜底文本，使 Faction.displayName、VictoryReason.displayName 和 DataLoader 旧战局校验错误改为迁移期中性口径，且不改变 enum rawValue、JSON schema、id、兼容查找、胜负判定、加载顺序、命令管线、规则或存档。v3.7-preflight.87 已收口静态 GameState/MapState fallback 可见文本，使最后兜底单位、地点、目标和初始化战报改为迁移期中性口径，且不改变 scenario id、objective id、faction、坐标、地形、补给源、胜负规则、加载顺序、命令管线或存档。v3.7-preflight.88 已收口 legacy objective lookup 字面量，使 `VictoryRules`、`RegionVictoryRules` 和 `MockAIClient` 的旧 fallback 目标查找改为 objective id 优先加中性旧战局名 fallback，且不改变 objective id、胜负阈值、AI 策略、`VictoryState` 字段、`VictoryReason` case、命令管线或存档。v3.7-preflight.89 已对齐 `RegionVictoryRules` 隋唐胜负摘要，使 `RegionRuleSystem` 的 region 层分析摘要按 `scenarioId` 分支，`wude_618` 读取洛阳、洛口仓、潼关和长安 objective id，且不改变主 `VictoryRules` 执行路径、`GameState.victoryState`、胜负阈值、命令管线或存档。v3.7-preflight.90 已抽出共享隋唐胜负 evaluator，使 `VictoryRules` 和 `RegionVictoryRules` 复用规则层 `Wude618VictoryEvaluator`，降低主胜负状态与 region 分析摘要分叉风险，且不改 objective id、胜负阈值、命令管线、AI 决策或存档。v3.7-preflight.91 已让 `CommandResultSummary` 记录 `commandKind` 命令语义，并让 `ZoneCommanderAgent` 最近静态防御判定按 `.hold` 判断，不再依赖 `Hold` 或“坚守”展示文本，且不改 `Command` case、directive schema、AI tactic 阈值、规则执行或存档主结构。v3.7-preflight.92 已把 `GamePhase.displayName` 自动阶段显示收口为朝堂行动 / 朝堂军令，并把 legacy `general_agents.json` 展示名改为历史总管，且不改 `GamePhase` rawValue、legacy id、势力 rawValue、单位 id、AI 决策、命令管线、规则或存档。
...
2. 读 `md/prompt/v3.0-隋唐迁移/v3.0_audit_and_contract.md`、`v3.1_powers_diplomacy_record.md`、`v3.2_scenario_map_record.md`、`v3.3_war_rules_record.md`、`v3.4_agent_court_record.md`、`v3.5_player_command_ux_record.md`、`v3.6_ui_art_polish_record.md`、`v3.7_victory_runtime_record.md`、`v3.7_lifecycle_save_record.md`、`v3.7_marshal_pure_json_output_record.md`、`v3.7_ui_record_sanitizer_helper_record.md`、`v3.7_legacy_general_court_visible_text_record.md`、`v3.7_command_panel_message_text_record.md`、`v3.7_legacy_unit_detail_tooltip_text_record.md`、`v3.7_legacy_region_detail_location_objective_text_record.md`、`v3.7_legacy_general_panel_record_summary_text_record.md`、`v3.7_map_display_adapter_spritekit_legacy_text_record.md`、`v3.7_mapeditor_selector_status_text_record.md`、`v3.7_appcontainer_interaction_text_record.md`、`v3.7_event_log_source_sanitizer_record.md`、`v3.7_agent_prompt_alias_sanitizer_record.md`、`v3.7_legacy_marshal_fallback_config_record.md`、`v3.7_court_diplomacy_record_id_sanitizer_record.md`、`v3.7_marshal_visible_title_sanitizer_record.md`、`v3.7_source_legacy_chinese_text_sanitizer_record.md`、`v3.7_general_panel_title_sanitizer_record.md`、`v3.7_legacy_fallback_json_visible_data_text_record.md`、`v3.7_legacy_source_fallback_visible_text_record.md` 和 `v3.7_legacy_static_fallback_source_text_record.md`。
3. 审计工作树和分支，不回滚用户改动。
4. v3.7 发布候选前置说明、最小外交关系命令、玩家侧州郡经营命令、AI 太守主动经营、AI 使者主动外交、归附事件记录链、归附空势力轮转收口、归附实体盘点、归附实体交接命令、归附交接审计记录、AI 归附实体交接、归附善后压力只读记录、AI 善后太守优先治理、善后处置审计记录、善后处置进度摘要、AI 善后未处置优先治理、善后完成状态提示、发布检查门禁拆分、AI 太守跳过诊断、AI 使者/交接跳过诊断、MapEditor 默认隋唐资源桥、地点字段化编辑、发布候选静态门禁快照、多轮玩家可见文案和记录 id 展示净化已基本完成；下一步优先处理共享展示 sanitizer 抽取、发布候选运行时清单或在人工授权后做完整构建、设备启动、界面点击烟测、MapEditor 操作烟测和观察者多回合验证。
