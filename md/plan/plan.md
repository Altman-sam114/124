...
| v3.7-preflight.101 | 归附善后治安压力落地 | 归附交接后的 `SubmissionAftermathRecord.riskLevel` 会写入受影响州郡 `OccupationState`，提高治安压力并降低顺从 | 不新增忠诚、叛乱、贡赋、俘虏、安置模型，不改命令或存档 schema |
| v3.7-preflight.102 | 归附善后贡赋效率落地 | 受控州郡收入会按 `OccupationState` 折算贡赋效率，高抵抗降低丁口、军械、粮草收入，安民后恢复 | 不新增忠诚、叛乱、俘虏、安置模型，不改命令或存档 schema |
| v3.7-preflight.103 | 渡口港口粮道补给减免 | 粮道跨河时若两端有渡口、港口或海港地点，免除该段补给渡河额外成本 | 不改移动、战斗、水战、港口补给源、JSON schema 或命令管线 |
| v3.7-preflight.104 | MapEditor 河边数据往返保真 | 编辑器文档、默认资源桥和导出器保留 tile `riverEdges`，避免覆盖默认资源时清空河边 | 不新增河流编辑 UI，不自动镜像邻边，不改运行时移动、战斗、补给规则或 JSON schema |
| v3.7-preflight.105 | MapEditor 河边绘制/擦除入口 | 地块模式可点击六边形边绘制或擦除当前 hex 的河边，画布和信息面板显示结果 | 不自动镜像邻边，不改默认 JSON、运行时规则、补给算法或 JSON schema |
| v3.7-preflight.106 | 受控渡口港口补给投送点 | 己控渡口、港口、海港在 `SupplyRules` 内部作为战术补给 anchor，参与补给线和撤退判定 | 不写入 `MapState.supplySources`，不改部署点、省级补给、经济新兵落点、移动、战斗或水战 |
| v3.7-preflight.107 | 默认水路地点河边资产补录 | 默认 `wude_618` 为蒲津渡、孟津渡、洛口津补录贴近地点的 `riverEdges`，四个默认水路地点自身河边非空 | 只改 JSON 资产，不改 schema、运行时移动/战斗/补给规则，不自动双写邻边 |
| v3.7-preflight.108 | 己控渡口港口移动渡河减免 | `MovementRules.shortestPaths` 识别己控渡口、港口、海港，免除相邻非道路跨河移动额外成本 | 不改公开纯地形 `movementCost`、命令、JSON schema、补给、战斗、水战或部署点 |
| v3.7-preflight.109 | 己控港口海港生产部署点 | `EconomyRules.deploymentHex` 在合格州郡和永久补给源之后，把己控安全港口/海港作为生产部署 fallback | 不写入 `MapState.supplySources`，不改补给线、收入、移动、战斗、水战、命令或 JSON schema |
| v3.7-preflight.110 | 己控港口海港战略补给源 | `RegionSupplyRules.strategicSupplySources` 把己控港口/海港所在州郡作为战略补给源 | 不写入 `MapState.supplySources`，不改战术补给、收入、生产部署、移动、战斗、水战、命令或 JSON schema |
| v3.7-preflight.111 | 归附善后四类审计字段 | `SubmissionAftermathRecord` 记录忠诚压力、叛乱风险、俘虏整编数量和安置州郡候选，外交面板展示 | 不新增叛乱触发、俘虏处置、安置命令、资源扣减、额外归属转移或 JSON 数据 schema |
| v3.7-preflight.112 | 隔河近战水路控制校验 | 无远程支援的相邻隔河攻击需要己控渡口、港口或海港；水师/弓弩/器械等远程支援不受该近战门禁阻断 | 不新增水战命令、水域控制、运输/登船、AI 管线、移动/补给规则或 JSON schema |
| v3.7-preflight.113 | 玩家可见接触态势术语古代化 | UI、战报、AI 自由文本和发布检查继续把“前线/战线”展示口径转为接触态势、边境压力、接敌、先阵等古代表达 | 不改 `FrontLine` / `FrontZone` 类型名、rawValue、schema、派生算法或存档字段 |
| v3.7-preflight.114 | 骑兵冲击与机动强化 | 骑兵基础攻击/移动提高，平原骑兵冲击加成提高，突出隋唐古代战场骑军差异 | 不改单位模板 JSON、AI tactic 权重、将领技能消费、水战、补给或命令 schema |
| v3.7-preflight.115 | 水路补给 anchor 粮道显示对齐 | `BoardScene` 绘制粮道虚线时复用 `SupplyRules.effectiveSupplyAnchors`，己控渡口/港口/海港可作为地图粮道落点 | 不改补给判定、JSON schema、移动、战斗、水战或部署；虚线仍非真实逐格路径 |
| v3.7-preflight.116 | 名将军令署名显示 | 军令面板标题转为“名将军令”，预备军令和提交反馈用已有将领 id 显示“某将军令” | 不改 `PlayerPlannedOperation` 存档字段、命令管线、AI 决策、规则执行、JSON schema 或将领技能消费 |
| v3.7-preflight.117 | AI 粮草目标权重 | AI 进攻/突破目标、模拟元帅摘要、legacy prompt 和 MockAI 评分更重视高粮草/粮仓州郡，并输出粮仓粮道理由 | 不改命令 schema、补给规则、战斗/移动、水战、部署、JSON 数据或真实 LLM 接入 |
...
当前已新增 v3.0-v3.7-preflight.117 迁移记录：
...
v3.7-preflight.44-.95 已连续收口 legacy 可见文案、记录净化、胜负摘要共享、命令结果语义化、阶段/总管展示口径、自动总管默认风格、默认风格共享 helper 和 DataLoader 场景阶段兜底。v3.7-preflight.96 已让 `GamePhase.normalized` 集中规范化旧 phase 存档语义，加载、App 状态切换、命令校验、回合推进和自动回合判断复用同一口径，合法 legacy 阿登阶段仍保留，隋唐或自定义脏 phase 会落到通用玩家/朝堂军令阶段，且不改 GamePhase rawValue、Codable、命令 schema、规则或存档字段。v3.7-preflight.97 已让 `WarCommandExecutor.applyStrategicAdvance` 不再把异常缺 zone 的动态方面推进势力静默兜底为旧东路势力，改为 zone faction / 行动军队推断，缺失则跳过并记录原因。v3.7-preflight.98 已让非 legacy `RegionDataSet` 缺 owner 时抛出校验错误，不再静默兜底旧西路势力。v3.7-preflight.99 已用 `ScenarioSemantics` 收口默认场景语义和胜负 fallback 门禁。v3.7-preflight.100 已让 MapEditor 默认资源导入遇到非法 unit faction 时跳过坏 unit 并生成可见诊断。v3.7-preflight.101 已让归附交接后的善后压力实际写入州郡治安/顺从状态。v3.7-preflight.102 已让治安/顺从折算受控州郡贡赋效率。v3.7-preflight.103 已让渡口、港口和海港地点减免粮道跨河补给成本。v3.7-preflight.104 已让 MapEditor 默认资源读写保留 tile 河边数据。v3.7-preflight.105 已让 MapEditor 地块模式可绘制/擦除当前 hex 河边。v3.7-preflight.106 已让己控渡口、港口和海港作为战术补给投送点。v3.7-preflight.107 已让默认 `wude_618` 四个水路地点自身 hex 均有非空 `riverEdges`。v3.7-preflight.108 已让己控渡口、港口和海港免除相邻非道路跨河移动额外成本。v3.7-preflight.109 已让己控港口/海港作为生产完成后的安全部署 fallback。v3.7-preflight.110 已让己控港口/海港所在州郡作为 region 战略补给源。v3.7-preflight.111 已让归附善后记录带出忠诚压力、叛乱风险、俘虏整编数量和安置州郡候选。v3.7-preflight.112 已让无远程支援的相邻隔河攻击需要己控水路通行点。v3.7-preflight.113 已把玩家可见“前线/战线”首轮转为接触态势、边境压力、接敌、先阵等古代口径。v3.7-preflight.114 已强化骑兵基础机动与平原冲击。v3.7-preflight.115 已让地图粮道虚线复用规则层永久补给源和己控水路补给 anchor。v3.7-preflight.116 已让预备军令和提交反馈显示“某将军令”。v3.7-preflight.117 已让 AI 进攻/突破目标、模拟元帅摘要、legacy prompt 和 MockAI 评分更重视高粮草/粮仓州郡，完整叛乱触发、俘虏处置、安置命令、名将技能消费、真实截粮/粮道封锁和更完整水战/水域控制仍待后续独立切片。
...
2. 读 `md/prompt/v3.0-隋唐迁移/v3.0_audit_and_contract.md`、`v3.1_powers_diplomacy_record.md`、`v3.2_scenario_map_record.md`、`v3.3_war_rules_record.md`、`v3.4_agent_court_record.md`、`v3.5_player_command_ux_record.md`、`v3.6_ui_art_polish_record.md`、`v3.7_victory_runtime_record.md`、`v3.7_lifecycle_save_record.md`，以及 `.88-.107` 最新记录：`v3.7_legacy_objective_lookup_text_record.md`、`v3.7_region_victory_suitang_alignment_record.md`、`v3.7_shared_victory_evaluator_record.md`、`v3.7_command_result_semantic_kind_record.md`、`v3.7_phase_legacy_agent_display_record.md`、`v3.7_agent_default_command_style_record.md`、`v3.7_command_style_helper_record.md`、`v3.7_dataloader_phase_fallback_record.md`、`v3.7_legacy_phase_normalization_record.md`、`v3.7_dynamic_theater_faction_fallback_record.md`、`v3.7_region_dataset_ownership_fallback_record.md`、`v3.7_scenario_semantics_victory_gating_record.md`、`v3.7_mapeditor_unit_faction_import_diagnostic_record.md`、`v3.7_submission_aftermath_occupation_pressure_record.md`、`v3.7_submission_aftermath_tribute_efficiency_record.md`、`v3.7_water_transit_supply_record.md`、`v3.7_mapeditor_river_edges_roundtrip_record.md`、`v3.7_mapeditor_river_edges_editing_record.md`、`v3.7_water_transit_supply_anchor_record.md`、`v3.7_water_transit_river_edges_asset_record.md`。
3. 审计工作树和分支，不回滚用户改动。
4. v3.7 发布候选前置说明、外交/州郡命令、AI 太守/使者/交接、归附善后记录、多轮玩家可见文案、记录 id 净化、胜负摘要共享、默认指挥风格、legacy phase 规范化、动态方面推进势力兜底、RegionDataSet owner 校验、场景语义门禁、MapEditor 非法 unit faction 导入诊断、部分水路/渡河/隔河近战规则、名将军令署名和基础 AI 粮草目标权重已基本完成；下一步优先从古代作战口径重塑、骑兵与名将规则消费、真实截粮/粮道封锁、正式地图资产/截图复核、完整发布候选运行时与 CI artifact 验收、善后实际规则、完整水战/港口补给、真实模型接入中切片。
