...
本工程仍保留 `WWIIHexV0` 历史项目名和 Swift + SwiftUI + SpriteKit 技术骨架。当前目标是在不破坏已有 hex 战术权威、region 战略聚合、动态战区、接触态势派生层、部署和统一命令管线的前提下，逐步迁移为可发布的隋末唐初 AI Agent 历史策略游戏。

首发迁移目标暂定为 `天命开唐 Agent`，默认剧本是 `武德元年：关中河洛争衡`。玩家默认执掌唐，也可在基础设置中选择当前局势内可玩的势力；其他势力由本地 AI / 朝堂链路推进。

隋唐迁移不是把二战战线换皮成古代地图。古代作战不存在现代连续“战线”概念，后续玩家可见体验应围绕天下局势、边境接触、行军道、关隘、粮仓、渡口、城池和州郡争夺展开；底层 `FrontLine` / `FrontZone` 兼容名可以保留，但展示层和 AI 说明应逐步改为“接触态势、战场态势、边境压力、接敌、先阵”等古代口径。

作战模式要继续研究和修改，方向是突出骑兵强机动与冲击、粮草补给与粮仓节点、屯田和府库粮草压力，以及隋唐英雄人物在军令、战报、外交和天下局势中的存在感。可参考《三国志14》等古代战略游戏的局势、粮道、人物和据点体验，但只取设计启发，不照搬资产、文本或具体数值。
...
- `ComponentType.cavalry` 基础攻击从 6 提到 7，移动从 5 提到 6，防御保持 4。
- `CombatRules` 平原骑兵冲击加成从 `+0.2` 提到 `+0.3`，让骑军在开阔地更符合隋唐古代作战核心差异。
- 边界：不改单位模板 JSON、AI tactic 权重、将领技能消费、水战、补给或命令 schema。

### v3.7-preflight.113：玩家可见接触态势术语古代化

- 主游戏 UI、战报、AI 自由文本和发布检查中的玩家可见“前线/战线”口径，继续转为“接触态势、边境压力、接敌、先阵、接触地段”等古代作战表达。
- 底层 `FrontLine` / `FrontZone` 类型名、rawValue、schema 和历史兼容记录保留，不做高风险重命名。
- 边界：只改展示/说明和自由文本，不改动态战区、部署、前线派生算法、命令管线或存档字段。

### v3.7-preflight.112：隔河近战水路控制校验

- `CommandValidator` 会在无远程支援的相邻隔河攻击时检查攻击方是否控制攻击端或防守端的渡口、港口或海港。
- 缺少己控水路通行点时返回 `waterCrossingBlocked`；水师、弓弩和器械等远程支援攻击不被该近战门禁阻断。
- 边界：不新增水战命令、水域控制模型、运输/登船、AI 管线改动或 JSON schema。

### v3.7-preflight.111：归附善后四类审计字段

- `SubmissionAftermathRecord` 现在记录忠诚压力、叛乱风险、俘虏整编数量和安置州郡候选。
- 外交面板善后区展示这四项，只作为归附交接后的复盘和后续治理提示。
- 边界：不新增叛乱触发、俘虏处置命令、安置命令、资源扣减、额外归属转移或 JSON 数据 schema。

### v3.7-preflight.110：己控港口海港战略补给源

- `RegionSupplyRules` 会把己方控制的港口/海港所在州郡作为战略补给源。
- 港口/海港必须可通行且由该势力控制；渡口不作为州郡级战略补给源。
- 边界：不写入 `MapState.supplySources`，不改战术补给、经济收入、生产部署、移动、战斗、水战、命令或 JSON schema。

### v3.7-preflight.109：己控港口海港生产部署点

- `EconomyRules` 在生产完成部署时，会把己方控制且安全的港口/海港作为后方部署 fallback。
- 港口/海港部署点只在没有合格州郡或永久补给源部署点时使用；渡口不作为募兵部署点。
- 边界：不写入 `MapState.supplySources`，不改补给线、移动、战斗、水战、经济收入、命令或 JSON schema。
...
- `SupplyRules` 内部会把己方控制的渡口、港口、海港地点作为补给路径 anchor。
- 该 anchor 只参与单位补给线、安全撤退和撤退排序，不写入 `MapState.supplySources`。
- 边界：不改部署点、省级补给、经济新兵落点、JSON schema、移动、战斗或水战。

### v3.7-preflight.105：MapEditor 河边绘制/擦除入口

- 地块模式新增“绘制河边 / 擦除河边”动作，点击六边形最近边修改当前 hex 的 `riverEdges`。
- MapEditor 画布会显示当前记录的蓝色河边，信息面板会列出选中地块已有河边方向。
- 边界：不自动镜像邻接边，不改默认 JSON，不改运行时移动、战斗、补给或 JSON schema。

### v3.7-preflight.104：MapEditor 河边数据往返保真

- `MapEditorHex` 现在保存 `riverEdges`，旧 `.mapeditor` 文档缺字段时按空集合兼容读取。
- 默认资源桥会把 scenario tile 的 `riverEdges` 导入编辑器文档；导出器会按稳定方向顺序写回 JSON。
- 边界：不新增河流绘制/编辑 UI，不自动镜像邻接边，不改 JSON schema、运行时河流规则、移动、战斗或补给算法。

### v3.7-preflight.103：渡口港口粮道补给减免落地

- `SupplyRules.supplyPathCost` 跨河计算粮道成本时，会读取两端 hex 的渡口、港口或海港地点标识。
- 若跨河两端任一处有水路通行点，该段补给渡河额外成本免除；普通跨河补给仍保留额外成本。
- 边界：不改移动、战斗、水师、水战、港口补给源、JSON schema、命令管线或存档字段。
...
- 覆盖整补、火力支援、突破、沿路推进、固守、接敌整补、收紧包围、接敌进攻、纵深驰援、驻防和战役预备等姿态。
...
- 善后治安压力、贡赋效率和忠诚/叛乱/俘虏/安置只读审计已接入；实际叛乱触发、俘虏处置和安置命令仍待后续。
...
- 归附后的治安压力、贡赋效率和忠诚/叛乱/俘虏/安置只读审计已有最小规则；实际叛乱触发、俘虏处置和安置命令仍未实现。
